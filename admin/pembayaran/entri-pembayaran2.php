...
$petugas = $admin->getPetugasById($_SESSION["id"]);

if (isset($_POST['bayar'])) {
  if ($admin->entriPembayaran($_POST) > 0) {
    echo
    "<script>
...
if (!isset($siswa["nisn"])) {
  ...
  $pembayaran =  $admin->getPembayaranByNisn($siswa["nisn"]);
  ...
  $jumlah_bayar = $admin->getSppById($siswa["id_spp"]);
}
?>
    ...
    <div class="card-body">
      ...
      <?php else : ?>
        <form action="" method="post" class="form mt-5">
          <input type="hidden" name="id_petugas" id="id_petugas" autocomplete="off" value="<?= $petugas["id_petugas"] ?>">
          <input type="hidden" name="nisn" id="nisn" autocomplete="off" value="<?= $siswa["nisn"] ?>">

          <div class="form-group">
            <label for="petugas">Petugas</label>
            <input type="text" name="petugas" id="petugas" class="form-control" required autocomplete="off" readonly value="<?= $petugas["nama_petugas"] ?>">
          </div>
          <div class="form-group">
